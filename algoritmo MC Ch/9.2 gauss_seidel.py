def gauss_seidel(A, b, e, max_i):
    ...
    for k in range(max_i):
        ...
        # Cria uma cópia do vetor x para não modificar a solução dentro do laço de cálculo.
        # Em Gauss-Seidel, o vetor é atualizado diretamente após calcular cada valor de x[i].
        x_novo = x[:]  # x_novo armazena os novos valores que serão usados na próxima iteração.
        ...
        for i in range(n):
            ...
            # Para Gauss-Seidel, devemos utilizar os valores mais recentes de x_novo imediatamente,
            # logo após calcular um valor para x[i].
            for j in range(n):
                # Se a variável j não for igual à variável i, soma o valor de A[i][j] * x_novo[j].
                if i != j:
                    soma -= A[i][j] * x_novo[j]  # Subtrai o valor de A[i][j] multiplicado pelo valor de x_novo[j].
...
# Exemplo de uso do método de Gauss-Seidel:

# Matriz dos coeficientes (sistema de equações lineares).
A = [
    [10, 2, 1],  # Primeira equação: 10x1 + 2x2 + x3 = 7
...
# Chama a função gauss_seidel com os parâmetros definidos.
resultado = gauss_seidel(A, b, e, max_i)  # A função retorna o vetor solução aproximada para o sistema.

# Imprime a solução calculada.
print("Solução:", resultado)  # Exibe a solução final após a execução do método de Gauss-Seidel.

"""
Comentários gerais sobre o código:
===================================
1. **Método de Gauss-Seidel:**
   - Gauss-Seidel é um método iterativo para resolver sistemas de equações lineares. Ao contrário do método de Gauss-Jacobi, onde a solução de cada variável depende apenas das iterações anteriores, Gauss-Seidel usa a solução mais recente para calcular as variáveis imediatamente após cada cálculo. Isso pode resultar em uma convergência mais rápida em alguns casos.

2. **Fórmula de atualização:**
   - A fórmula de atualização usada no método de Gauss-Seidel para cada variável é dada por:
     x_i^(k+1) = (b_i - somatório(A[i][j] * x_j)) / A[i][i]
   - Ou seja, a solução de uma variável depende das soluções das outras variáveis na iteração atual e anterior.
...
5. **Exemplo prático:**
   - O exemplo fornecido resolve o sistema linear Ax = b usando o método de Gauss-Seidel, onde A é uma matriz 3x3 e b é um vetor de 3 elementos. A solução final é impressa após a convergência do método ou após o número máximo de iterações.

6. **Eficiência:**
   - O método de Gauss-Seidel pode ser mais eficiente que o método de Gauss-Jacobi em termos de convergência, pois usa imediatamente as soluções mais recentes. No entanto, como qualquer método iterativo, ele pode ser lento em sistemas grandes ou quando a matriz não é bem condicionada.
"""
